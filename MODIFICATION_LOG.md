...
训练前应检查两份 `.jsonl.meta.json` 的 `records=81381`、`min_token_count_with_eos=max_token_count_with_eos=6144`、`total_token_count_with_eos=500004864`；合并文件应为 `162762` 行。首个 LongCrawl 分片已在远程服务器，无需重复下载；若实际不够，可从发布方增加分片，按顺序传给多值 `--input`。
